...
import static com.service.config.RabbitConfig.ORDER_CREATED_KEY;
...
public class RabbitAdapterService {
    ...
    /**
     * Отправить ивент о созданном заказе в billing сервис.
     */
    public void sendOrderCreatedEvent(Order order) {
        try {
            String payload = objectMapper.writeValueAsString(
                    new OrderCreatedEvent(order.getUserId(),
                            order.getId(),
                            order.getAmount()
                    )
            );

            rabbitTemplate.convertAndSend(ORDER_EVENTS_TOPIC_EXCHANGE, ORDER_CREATED_KEY, payload);
